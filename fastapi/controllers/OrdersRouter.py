...
async def get_order_by_id(order: OrderSchema, db: Session = Depends(get_db)):
    loggedin_user = getLoggedInUser()
    order.user_id = loggedin_user["user_id"]
    
    # create new order object and commit in the database
    new_order = OrderModel(id = str(shortuuid.uuid()),total = order.total, track_id = order.track_id, user_id = loggedin_user["user_id"], shop_id = order.shop_id, rating = order.rating)
    await orderService.commit_new_order(db, new_order)
    
    # iterate over the product list and add product order mapping
    for product in order.products:
        new_product_order = OrderProductMapperModel(product_id= product.id, order_id= new_order.id, quantity= product.quantity, available= True)
        await orderService.commit_new_op_mapper(db, new_product_order)
...
async def update_order_rating(order: OrderSchema, db: Session = Depends(get_db)):
    loggedin_user = getLoggedInUser()
    order_item = await orderService.get_order_by_id(db, id)
    
    if not order_item:
        logger.info("No order was found")
        raise HTTPException(status_code=404, detail=f'order with id: {order.id} not found')
    
    if not order.feedback:
        new_feedback = FeedbackModel(id = str(shortuuid.uuid()), feedback = order.feedback, user_id = loggedin_user["user_id"], shop_id = order.shop_id, order_id = order.id)
        db.add(new_feedback)
        db.commit()
...
async def update_order_rating(order: OrderSchema, db: Session = Depends(get_db)):
    ...
    for product in unavailable_products:
        product_item = await orderService.get_order_product_mapping(db, product.id, order.id)
        
        if not product_item:
            logger.info("No product-order mapping was found")
            raise HTTPException(status_code=404, detail=f'order-product mapping with product id: {product_item.product_id} & order id: {order.id} not found')
        ...
        if product.quantity == product_item.quantity:
            ...
            await orderService.delete_op_mapping(db, product.id, order.id, product_item.quantity)
