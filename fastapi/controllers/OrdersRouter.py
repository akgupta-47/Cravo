...
async def create_new_order(order: OrderSchema, db: Session = Depends(get_db)) -> OrderSchema:
    loggedin_user = getLoggedInUser()
    order.user_id = loggedin_user["user_id"]
    
    # create new order object and commit in the database
    new_order = OrderModel(id = str(shortuuid.uuid()),
                           total = order.total, track_id = order.track_id, 
...
async def update_order_rating(order: OrderSchema, db: AsyncSession = Depends(get_db)) -> OrderSchema:
    loggedin_user = getLoggedInUser()
    # Fetch the order
