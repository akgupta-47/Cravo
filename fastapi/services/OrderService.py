...
async def get_all_orders_by_user(
    db: AsyncSession, loggedin_user: str
) -> List[OrderModel]:
...
async def get_order_product_mapping(
    db: AsyncSession, product_id: str, order_id: str
) -> Optional[OrderProductMapperModel]:
    stmt = select(OrderProductMapperModel).filter(
...
async def delete_op_mapping(
    db: AsyncSession, product_id: str, order_id: str, quantity: int
) -> None:
    """Delete a product-order mapping."""
    stmt = delete(OrderProductMapperModel).where(
        OrderProductMapperModel.product_id == product_id,
        OrderProductMapperModel.order_id == order_id,
